...
class RBTree:
            ...
            if (
                ...
                if (
                    ...
                    k.parent.parent.color = (
                        1  # set color of grandparent as Red
                    )
                    k = (
                        k.parent.parent
    ...
    # Function to graph
    def __tograph(self, node):
        G = nx.DiGraph()
        if node != self.NULL:
            s_color = "red" if node.color == 1 else "black"
            G.add_node(node.val, color=s_color)
            # print ( f'add_node {node.val}, color = {s_color}' )
            # if node.right.val != 0:
            #    print (f'add_edge {node.val} -R-> {node.right.val}')
            # if node.left.val != 0:
            #    print (f'add_edge {node.val} -L-> {node.left.val}')

            leftG = self.__tograph(node.left)
            rightG = self.__tograph(node.right)
            if leftG != None:
                G = nx.union(G, leftG)
    ...
    def tograph(self):
        return self.__tograph(self.root)

    # -----------------------------------------
    # exists()
    @profile
    def exists(self, node, val):
        if val == node.val:
            return True
        ...
        if val < node.val:
            if node.left == None:
                return False
            return self.exists(node.left, val)

        if node.right == None:
            return False
        return self.exists(node.right, val)
